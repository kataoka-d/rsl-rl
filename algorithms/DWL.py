...
from rsl_rl.models import DWLRNNModel
...
class DWL:
    ...
    def __init__(
        ...
        self.denoising_loss_coef = denoising_loss_coef
    ...
    def update(self) -> dict[str, float]:
        ...
        mean_denoising_reconstruction_loss = 0
        mean_denoising_latent_l1 = 0
        # RND loss
        ...
        for batch in generator:
            ...
            denoising_info = None
            if hasattr(self.actor, "compute_denoising_loss"):
                denoising_info = self.actor.compute_denoising_loss(batch.observations, masks=batch.masks)
            if denoising_info is not None:
                loss += self.denoising_loss_coef * denoising_info["loss"]
            ...
            if denoising_info is not None:
                mean_denoising_loss += denoising_info["loss"].item()
                mean_denoising_reconstruction_loss += denoising_info["reconstruction"].item()
                mean_denoising_latent_l1 += denoising_info["latent_l1"].item()
            # RND loss
        ...
        mean_denoising_loss /= num_updates
        mean_denoising_reconstruction_loss /= num_updates
        mean_denoising_latent_l1 /= num_updates
        if mean_rnd_loss is not None:
        ...
        if hasattr(self.actor, "denoising_decoder") and self.actor.denoising_decoder is not None:
            loss_dict["denoising"] = mean_denoising_loss
            loss_dict["denoising_reconstruction"] = mean_denoising_reconstruction_loss
            loss_dict["denoising_latent_l1"] = mean_denoising_latent_l1
        if self.rnd:
    ...
    @staticmethod
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> DWL:
        """Construct the DWL algorithm."""
        # Resolve class callables
        alg_class: type[DWL] = resolve_callable(cfg["algorithm"].pop("class_name"))  # type: ignore
        ...
        alg: DWL = alg_class(actor, critic, storage, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"])
