...
from rsl_rl.models.CustomMLPmodel import CustomMLPModel
...
class CustomPPO:
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            z_history = self.actor.encode_history(batch.observations) #hisotryをencode
            z_privilege = self.critic.output_privilege_info(batch.observations)
            latent_aligment_loss = criterion(z_history, z_privilege.detach())

            loss = surrogate_loss + self.value_loss_coef * value_loss - self.entropy_coef * entropy.mean() + latent_aligment_loss * self.alignment_loss_coef
    ...
    def get_policy(self) -> CustomMLPModel:
        """Get the policy model."""
        return self.actor

    @staticmethod
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> CustomPPO:
        """Construct the PPO algorithm."""
        # Resolve class callables
        alg_class: type[CustomPPO] = resolve_callable(cfg["algorithm"].pop("class_name"))  # type: ignore
        actor_class: type[CustomMLPModel] = resolve_callable(cfg["actor"].pop("class_name"))  # type: ignore
        ...
        alg: CustomPPO = alg_class(actor, critic, storage, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"])
