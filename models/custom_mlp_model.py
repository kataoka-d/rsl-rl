...
class CustomModel(nn.Module):
    ...
    def __init__(
        ...
        encode_privilege_obs : bool = False,
        encoder_hidden_dims: tuple[int, ...] | list[int] = (256, 256),
        encoder_output_dim : int = 47,
    ) -> None:
        ...
        """
        super().__init__()
        self.obs_name = obs_set #個人的に追加した
        ...
        self.encoder = MLP(self._get_latent_dim(), encoder_output_dim, encoder_hidden_dims, activation)
        ...
        #privilege encoder
        #仮の設定　
        if encode_privilege_obs:
            privilege_encoder_layers = []
            num_privilege_encoder_input = 2
            num_privilege_encoder_latent = 2
            ...
            for l in range(len(privilege_encoder_hidden_dims)):
                if l == len(privilege_encoder_hidden_dims) -1:
                    privilege_encoder_layers.append(
                        nn.Linear(privilege_encoder_hidden_dims[l], num_privilege_encoder_latent))
                else:
    ...
    def forward(
        ...
        expected_keys = {"policy", "actor_history"}

        if set(obs.keys()) != expected_keys:
            raise KeyError(f"Invalid obs keys: {obs.keys()}. "f"Expected only {expected_keys}"
            )
        
        prop_obs = obs['policy']
        encode_obs = obs['actor_history']
        
        #obsの中に特定のkeyが入っているかどうか
        if 'policy' in obs:
            print(obs['policy'])
            
        #特定のkeyによる条件分岐の確認
        #この例を参考にencoder,decoderの処理を加える

        #エンコーダーに通してからpolicyの観測と連結する
        encode_output = self.encoder(self.get_latent(encode_obs, masks, hidden_state))

        # Get MLP input latent
        latent = self.get_latent(prop_obs, masks, hidden_state)

        mlp_input = torch.cat([latent, encode_output],dim=-1)
...
class _OnnxMLPModel(nn.Module):
    ...
    def __init__(self, model: CustomModel, verbose: bool) -> None:
