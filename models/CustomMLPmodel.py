...
class CustomMLPModel(nn.Module):
    ...
    def __init__(
        ...
        encoder_output_dim: int | int = 147, #実際は特権情報の次元数
        history_dim : int | int = 310, #観測62次元×5step → train_cfgから指定できるようにする
    ) -> None:
        ...
        self.obs_groups, self.obs_dim, policy_dim = self._get_obs_dim(obs, obs_groups, obs_set)

        # Observation normalization
        self.obs_normalization = obs_normalization
        if obs_normalization:
            self.obs_normalizer = EmpiricalNormalization(policy_dim)
        else:
            self.obs_normalizer = torch.nn.Identity()

        self.latent_normalizer = nn.LayerNorm(self.encoder_output_dim)
        ...
        self.mlp_encoder = MLP(history_dim, self.encoder_output_dim, encoder_hidden_dims, activation)
    ...
    def forward(
        ...
        obs_encode = obs['actor_history']

        #Get encoder input latent
        encode_latent = self.mlp_encoder(obs_encode)

        #連結前に正規化
        obs_policy = self.obs_normalizer(obs_policy)
        encode_latent = self.latent_normalizer(encode_latent)
        #連結
        latent = torch.cat( [obs_policy, encode_latent],dim=-1)
    ...
    def encode_history(self, obs: TensorDict) -> torch.Tensor:
        z = self.mlp_encoder(obs["actor_history"])
    ...
    def _get_obs_dim(self, obs: TensorDict, obs_groups: dict[str, list[str]], obs_set: str) -> tuple[list[str], int]:
        ...
        self.obs_dim = policy_dim + encoder_out_dim


        return ["policy", "actor_history"], self.obs_dim, policy_dim
...
class _TorchMLPModel(nn.Module):
    ...
    def __init__(self, model: CustomMLPModel) -> None:
...
class _OnnxMLPModel(nn.Module):
    ...
    def __init__(self, model: CustomMLPModel, verbose: bool) -> None:
